...
A manual is a simple markdown file which contain some instructions about a specific step, and can be found in the `manuals` dir. The only exception is the first manual which should give us an introduction to the tutorial and is written in the `manuals/README.md` file. The rest of the manuals should be located in the `manuals/steps` dir in a file called `step(index).md` (`index` represents the index of the step). The manual files are auto-generated by Tortilla using the [render command](#render-manual) and will be outputted in the project's root dir (e.g `README.md` and `steps/step1.md`). The manual files are provided with some handy template helpers which will ease the process manuals writing, as listed in the following:
